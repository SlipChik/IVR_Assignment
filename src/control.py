...
class kinematics:
    def __init__(self):
    	...
    	self.target_end_effector = np.array([0.0, 0.0, 0.0], dtype='float64')
    ...
    def get_jacobian(self, q1, q3, q4):
        ...
        xq1 = - 2.8*np.sin(q1)*np.sin(q4) + 2.8*np.cos(q1)*np.cos(q4)*np.sin(q3) + 3.2*np.cos(q1)*np.sin(q3) 
    ...
    def pid_control(self, target, current): 
    	...
    	jacobian = get_jacobian(self.joints[0], self.joints[1], self.joints[2])
    	J_inv = np.linalg.pinv(jacobian) 

    	target_joints_d =  self.get_pid_r((target-current), J_inv)
    	target_joints = self.joints + target_joints_d * dt  #50
    ...
    def callback(self, data):
    	...
    	self.target_end_effector = float(data)

    	# x = Float64()
        # y = Float64()
        # z = Float64()
    	current_end_effector = get_FK_matrix(joints[0], joints[1], joints[2])
    	# x = current_end_effector[0]
    	# y = current_end_effector[1]
    	# z = current_end_effector[2]

    	pub_current_end_effector = Float64MultiArray()
    	pub_current_end_effector.data = np.array([current_end_effector[0], current_end_effector[1], current_end_effector[2]])

    	target_joints = self.pid_control(self.target_end_effector, current_end_effector)
    	joint1 = Float64()
    	joint3 = Float64()
    	joint4 = Float64()
    	joint1 = target_joints[0]
    	joint3 = target_joints[1]
    	joint4 = target_joints[2]



        # Publish the results
        try:
            self.end_effector_cal_pub.publish(pub_end_effector_pos)
