...
class image_converter:
    ...
    def trajectory(self):
        ...
        self.tx = -3.7704095373962643 # float(3.0 * np.cos(cur_time * np.pi / 20))
        self.ty = -3.0844328092945603 # float(4.0 * np.sin(cur_time * np.pi / 14) + 0.5)
        self.tz = 3.5673860232518364 # float(1.0 * np.sin(cur_time * np.pi / 18) + 4.5)
    ...
    def forward_kinematics(self,image1,image2):
        ...
        end_effector = np.array(
                                [link3_length*np.cos(joints[0])*np.sin(joints[2])+np.sin(joints[0])*np.sin(joints[1])*(link3_length*np.cos(joints[2])+link2_length),
    ...
    def control_open(self, image1, image2):
        ...
        print("d", (dt * np.dot(J_inv, self.error.transpose())))
        return q_d
